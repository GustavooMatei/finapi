...
describe('Get Balance', () => {
  beforeAll(async () => {
    ...
    await connection.query(`
      ...
      VALUES ('${user.id}', 'Gustavo Oliveira', '${user.email}', '${user.hashedPassword}', NOW(), NOW())
    `);

    await connection.query(`
      INSERT INTO users(id, name, email, password, created_at, updated_at)
      VALUES ('${otherUser.id}', 'Gustavo Oliveira', '${otherUser.email}', '${otherUser.hashedPassword}', NOW(), NOW())
    `);
